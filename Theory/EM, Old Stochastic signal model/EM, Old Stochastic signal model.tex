...
\begin{equation}
\mathbf{\Gamma}^{(\tau)} =  \frac{1}{\text{G}}  \hat{s}^{(\tau)} \cdot [\hat{s}^{(\tau)}]^*  + \Sigma_{S_t|X_t}.
\end{equation}
Предполагая, что сигналы некоррелированны, изменим оценку так, чтобы учесть лишь диагональные элементы:
\begin{equation}
\mathbf{\Gamma}^{(\tau)} =  \mathcal{D} \bigg[  \frac{1}{\text{G}}\hat{s}^{(\tau)} \cdot [\hat{s}^{(\tau)}]^*  + \Sigma_{S_t|X_t}\bigg].
\end{equation}
