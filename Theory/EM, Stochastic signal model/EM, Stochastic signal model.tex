...
\end{enumerate}
Альтернативный подход: используем $\hat{\text{R}}^{(0)}$ для оценки  $\theta^{(0)}$ с помощью подпространственного метода, может быть использован MUSIC, к примеру. \\
...
\begin{equation}
\Expect[S_t|X_{t,o_t}=x_{t, o_t}] =  \Expect[\mathbf{\Gamma}^* \text{A}^* \hat{\text{R}}^{-1} X_t|X_{t,o_t}=x_{t, o_t}] = \mathbf{\Gamma}^* \text{A}^* \hat{\text{R}}^{-1} \Expect[X_t|X_{t,o_t}=x_{t, o_t}].
\end{equation}
