...
1&1&\dots&1\\
e^{-2j\pi \frac{d}{\lambda}\sin(\theta_1)}& e^{-2j\pi \frac{d}{\lambda}\sin(\theta_2)}&\dots&e^{-2j\pi \frac{d}{\lambda}\sin(\theta_K)}\\
\dots&\dots&\ddots&\vdots\\
e^{-2j\pi (L-1) \frac{d}{\lambda}\sin(\theta_1)}& e^{-2j\pi (L-1) \frac{d}{\lambda}\sin(\theta_2)}&\dots&e^{-2j\pi (L-1) \frac{d}{\lambda}\sin(\theta_K)}\\
\end{bmatrix}.
\nonumber
\end{gather}
Сигналы, испускаемые источниками, также как и шумы на сенсорах, предполагаются стохастическими: $S_t \sim CN(\mathbf{O}_{K \times 1},\mathbf{\Gamma}),t=\overline{1,G}$, $N_t \sim CN(\mathbf{O}_{L \times 1}, \mathbf{\Lambda})$. Матрицы $\mathbf{\Gamma}$ и $\mathbf{\Lambda}$ предполагаются диагональными, т.е. и сигналы, и шумы, являются некоррелированными. Для простоты дальнейших рассуждений введем также следующие величины:
\begin{itemize}
\item
...
\end{center}
Воспользуемся EM-алгоритмом для того, чтобы определить значения параметров $\Psi = (\theta, \mathbf{\Gamma})$, пропущенные значения $X_m=\{X_{m_t}\}_{t=1}^G$ рассматриваются как латентные переменные. Наблюдения $X_t$, $t=\overline{1,L}$ предполагаются независимыми и одинаково распределенными.
\begin{center}
...
\begin{gather*}
X_t = AS_t + N_t \\
S_t \sim CN(\mathbf{O}_{M \times 1}, \mathbf{\Gamma}) \\
X_t \sim CN(\mathbf{O}_{L \times 1}, A\mathbf{\Gamma}A^* + \mathbf{\Lambda})\\
X_t|S_t \sim CN(AS_t,  \mathbf{\Lambda})\\
X_{o_t} \sim CN(\mathbf{O}_{L \times 1}, A_{o_t}\mathbf{\Gamma}A_{o_t}^* + \mathbf{\Lambda_{o_t}})\\
\end{gather*}
\begin{gather}
P(S|\Psi) = \prod_{t=1}^G \frac{1}{\pi^M \Det(\mathbf{\Lambda})}e^{-S_t^* (\mathbf{\Gamma})^{-1}S_t},
\end{gather}
\begin{gather}
P(X|\Psi) = \prod_{t=1}^G \frac{1}{ \pi^L \Det(A\mathbf{\Gamma}A^* + \mathbf{\Lambda}) } e^{-X_t^* (A\mathbf{\Gamma}A^* + \mathbf{\Lambda})^{-1}X_t},
\end{gather}
\begin{gather}
P(X|S,\Psi) = \prod_{t=1}^G \frac{1}{\pi^L \Det(\mathbf{\Lambda}) }e^{-(X_t-AS_t)^* (\mathbf{\Lambda})^{-1}(X_t-AS_t)},
\end{gather}
\begin{gather}
P(X_o|\Psi) = \prod_{t=1}^G \frac{1}{\pi^{L_{o_t}} \Det(\mathbf{\Lambda}_{o_t})}e^{-(X_{o_t})^* (\mathbf{\Lambda}_{o_t})^{-1}(X_{o_t})},
\end{gather}
Параметры апостериорного распределения $P(X_{m_t}|X_{o_t}=x_{o_t},\Psi)$ на итерации $\tau$ можно найти следующим образом:
\begin{equation}
\left\{ \begin{aligned} 
\mu_{X_{m_t}|X_{o,t}=x_{o_t}}^{(\tau)} &= \hat{\Sigma}_{X_{m_t},X_{o_t}}^{(\tau)}(\hat{\Sigma}_{X_{o_t},X_{o_t}}^{(\tau)})^{-1}\cdot x_{o_t} \\
\Sigma_{X_{m_t}|X_{o,t}=x_{o_t}}^{(\tau)} &= \hat{\Sigma}_{X_{m_t},X_{m_t}}^{(\tau)}-\hat{\Sigma}_{X_{m_t},X_{o_t}}^{(\tau)}(\hat{\Sigma}_{X_{o_t},X_{o_t}}^{(\tau)})^{-1}\hat{\Sigma}_{X_{o_t},X_{m_t}}^{(\tau)}
\end{aligned} \right.
\end{equation}
...
Находим новую оценку ковариации наблюдений $\hat{\Sigma}_{X}^{(\tau+1)}$ как выборочную ковариацию восстановленного набора данных.
...
\begin{equation}
\left\{ \begin{aligned} 
\mu_{S_t|X_{o_t} = x_{o_t}, X_{m_t} = \widetilde{x}_{m_t}^{(\tau)}, \Psi}^{(\tau)} &= \mathbf{\Gamma}A^*\Big(A\mathbf{\Gamma}A^*+\mathbf{\Lambda}\Big)^{-1}\widetilde{x}_t^{(\tau)} \\
\Sigma_{S_t|X_{o_t} = x_{o_t}, X_{m_t} = \widetilde{x}_{m_t}^{(\tau)}, \Psi}^{(\tau)} &= \mathbf{\Gamma} - \mathbf{\Gamma}A^*\Big(A\mathbf{\Gamma}A^*+\mathbf{\Lambda}\Big)^{-1}A\mathbf{\Gamma}
\end{aligned} \right.
\end{equation}
где $A=A^{(\tau-1)}, \mathbf{\Gamma}=\mathbf{\Gamma}^{(\tau-1)}$. \\
Заметим, что $\Sigma_{S_t|X_{o_t} = x_{o_t}, X_{m_t}=\widetilde{x}_{m_t}^{(\tau)}}$ не зависит от величин, которые зависят от $t$, эта условная ковариация исходных сигналов вообще не зависит от $t$.
...
\log P(X,S|\theta, \mathbf{\Gamma}) = \log P(X|S=s, \theta) + \log P(S|\mathbf{\Gamma}) = \\
 = -G \log (\Det(\pi \mathbf{\Lambda})) - \sum_{t=1}^G (X_t-A(\theta)S_t)^* \mathbf{\Lambda}^{-1}(X_t-A(\theta)S_t) 
\\ - G \log(\Det(\pi \mathbf{\Gamma})) - \sum_{t=1}^G S_t^* \mathbf{\Gamma}^{-1}S_t
\end{gathered}
\end{equation*}
\begin{center}
...
\argmax_{\Psi} \Expect_{(X_m,S)|X_o=x_o, \Psi^{(\tau-1)}}\bigg[-G \log (\Det(\pi \mathbf{\Lambda})) - \sum_{t=1}^G (X_t-A(\theta)S_t)^* \mathbf{\Lambda}^{-1}(X_t-A(\theta)S_t) 
\\ - G \log(\Det(\pi \mathbf{\Gamma})) - \sum_{t=1}^G S_t^* \mathbf{\Gamma}^{-1}S_t\bigg]
\end{gathered}
\end{equation*}
Оценим углы прибытия сигналов $\theta$:
\begin{equation*}
\begin{gathered}
\theta^{(\tau)}= \argmax_{\theta} Q(\theta | \theta^{(\tau-1)}) = \\
\argmax_{\theta} \Expect_{(X_m,S)|X_o=x_o, \Psi^{(\tau-1)}}\bigg[-G \log (\Det(\pi \mathbf{\Lambda})) - \sum_{t=1}^G (X_t-A(\theta)S_t)^* \mathbf{\Lambda}^{-1}(X_t-A(\theta)S_t) 
\\ - G \log(\Det(\pi \mathbf{\Gamma})) - \sum_{t=1}^G S_t^* \mathbf{\Gamma}^{-1}S_t\bigg]
\end{gathered}
\end{equation*}
...
\begin{equation*}
\begin{gathered}
\mathcal{J}(\theta) = \sum_{t=1}^G \Expect \Big[- (X_t-A(\theta)S_t)^*\mathbf{\Lambda}^{-1}(X_t-A(\theta)S_t) \text{|}X_o = x_o\Big] = \\
= - ||\mathbf{\Lambda}^{-1/2}(\widetilde{x}^{(\tau)}-A\widetilde{S})||_F^2.
\end{gathered}
\end{equation*}
Оценка УМО, полученная выше, была выведена в документе для детерминированной модели сигналов.
\begin{equation}
\theta^{(\tau)}=\argmin_{\theta} ||\mathbf{\Lambda}^{-1/2}(\widetilde{x}^{(\tau)}-A\widetilde{S})||_F^2
\end{equation}
...
\end{gathered}
\end{equation*}
Пользуемся тем фактом, что полное правдоподобие раскладывается на сумму $\log P(X|S=s) + \log(S)$. Первый логарифм не зависит от $\mathbf{\Gamma}$. Поэтому максимизируем условное математическое ожидание  для $\log(S| \Psi)$.
\begin{equation*}
\begin{gathered}
\mathcal{K}(\mathbf{\Gamma}) = \Expect_{S|X_=\widetilde{x}, \Psi^{(\tau-1)}}\bigg[- G \log(\Det(\pi \mathbf{\Gamma})) - \sum_{t=1}^G S_t^* \mathbf{\Gamma}^{-1}S_t\bigg] = \\
- G \log(\Det(\pi \mathbf{\Gamma})) - \Expect_{\Psi^{(\tau-1)}}\bigg[ \sum_{t=1}^G S_t^* \mathbf{\Gamma}^{-1}S_t \Big | X = \widetilde{x}^{(\tau)} \bigg ] = \\
- G \log(\Det(\pi)) - G \log(\Det(\mathbf{\Gamma})) - \Expect_{\Psi^{(\tau-1)}}\bigg[ \sum_{t=1}^G S_t^* \mathbf{\Gamma}^{-1}S_t \Big | X = \widetilde{x}^{(\tau)} \bigg ] = \\
 - G \log(\Det(\mathbf{\Gamma})) - \sum_{t=1}^G \Tr\Big(\mathbf{\Gamma}^{-1} \Expect[S_t S_t^* | X = \widetilde{x}^{(\tau)}]\Big)
\end{gathered}
\end{equation*}
\begin{equation*}
\begin{gathered}
\frac{d}{d\mathbf{\Gamma}}\log (\Det (\mathbf{\Gamma})) = \mathbf{\Gamma}^{-1}
\end{gathered}
\end{equation*}
$X = \widetilde{x}^{(\tau)}$ -- оценка наблюдений, полученная с учетом Е-шага текущей итерации.
Обозначим через $M$ величину $\Expect[S_t S_t^*\Big| X = \widetilde{x}^{(\tau)}]$.
\begin{equation*}
\begin{gathered}
\frac{d}{d\mathbf{\Gamma}}\Tr(P^{-1}M)= -\mathbf{\Gamma}^{-1}M\mathbf{\Gamma}^{-1}
\end{gathered}
\end{equation*}
\begin{equation*}
\begin{gathered}
\frac{d\mathcal{K}(\mathbf{\Gamma})}{d\mathbf{\Gamma}} = -G\mathbf{\Gamma}^{-1}+\mathbf{\Gamma}^{-1}M\mathbf{\Gamma}^{-1}
\end{gathered}
\end{equation*}
Приравняем производную к нулю (функция по $\mathbf{\Gamma}$ выпукла).
\begin{equation*}
\begin{gathered}
O = -G\mathbf{\Gamma}^{-1}+\mathbf{\Gamma}^{-1}M\mathbf{\Gamma}^{-1} \Rightarrow M =G\mathbf{\Gamma} \Rightarrow \mathbf{\Gamma}^{(\tau)} = \frac{1}{G} \sum_{t=1}^G \Big( \Sigma_{S_t|X_t} +  \mu_{S_t|X_t}  (\mu_{S_t|X_t})^* \Big)
\end{gathered}
\end{equation*}
\begin{equation}
\mathbf{\Gamma}^{(\tau)} = \frac{1}{G} \sum_{t=1}^G \Big( \Sigma_{S_t|X_t} +  \mu_{S_t|X_t}  (\mu_{S_t|X_t})^* \Big)
\end{equation}
Учтем, что $\sum_{t=1}^G a_t a_t^* = AA^*$, если $A$ -- матрица, составленная из столбцов $a_t, t=\overline{1,G}$. Также учтем, что $\Sigma_{S_t|X_t}$ принимает одинаковое значение при любом $t$:
\begin{equation}
\mathbf{\Gamma}^{(\tau)} =  \frac{1}{G}  M_{S|X} \cdot M_{S|X}^*  + \Sigma_{S_1|X_1},
\end{equation}
...
\begin{equation}
\mathbf{\Gamma}^{(\tau)} =  \mathcal{D} \bigg[  \frac{1}{G} M_{S|X} \cdot M_{S|X}^*  + \Sigma_{S_t|X_t}\bigg]
\end{equation}.
\begin{center}
