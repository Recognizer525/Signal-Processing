...
\Sigma_{S_t|\mathcal{I}_t, X_{t, m_t}} &= \mathbf{\Gamma} - \mathbf{\Gamma}^*\text{A}^*  \hat{\text{R}}^{-1}\text{A}\mathbf{\Gamma} + \mathbf{\Gamma}\text{A}^* \hat{\text{R}}^{-1}\text{Cov}(X_t|\mathcal{I}_t)\hat{\text{R}}^{-1} \text{A}\mathbf{\Gamma},
\end{aligned} \right.
\end{equation}
...
\end{equation}
Теперь найдем $\Expect[S_t|X_{t,o_t}=x_{t, o_t}]$, $\Expect[S_t|X_{t}]$:
\begin{equation}
\Expect[S_t|X_{t,o_t}=x_{t, o_t}] = \mathbf{\Gamma}^* \text{A}^* \hat{\text{R}}^{-1} \Expect[X_t|X_{t,o_t}=x_{t, o_t}],
\end{equation}
\begin{equation}
...
\begin{equation}
\text{Cov}(\Expect[S_t|X_t]|X_{t, o_t}=x_{t, o_t}) = \text{Cov}(\mathbf{\Gamma}^* \text{A}^* \hat{\text{R}}^{-1} X_t |X_{t, o_t}=x_{t, o_t}) = \mathbf{\Gamma}^* \text{A}^* \hat{\text{R}}^{-1} \text{Cov}(X_t | X_{t, o_t}=x_{t, o_t}) \hat{\text{R}}^{-1} \text{A} \mathbf{\Gamma}.
\end{equation}
...
\begin{equation*}
\begin{gathered}
\Expect[X_t S_t^*|X_{t, o_t}=x_{t, o_t}] = \Expect[X_t \Expect[S_t^*|X_t]|X_{t, o_t}=x_{t, o_t}] = \Expect[X_t  (\mathbf{\Gamma}^* \text{A}^* \hat{\text{R}}^{-1} X_t)^*  | X_{t, o_t}] \\ 
