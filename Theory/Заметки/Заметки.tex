...
\begin{equation}
\mathcal{Q}(\theta|\theta^{(\tau-1)}) = \Expect_{q(\Xi)}[\log P(X,S| \theta)]\\ + \log P(S).
\end{equation}
...
\begin{equation}
p(\mathbf{\Gamma} ) \propto |\mathbf{\Gamma} |^{-(\nu + d + 1)/2}
\exp\!\left(-\tfrac12 \operatorname{tr}(\Psi \mathbf{\Gamma} ^{-1})\right).
\end{equation}
\end{document}
