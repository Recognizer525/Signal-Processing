...
\end{equation*}
и $X_t \in \Complex^{\text{L}}, S_t \in \Complex^{\text{K}}, N_t \in \Complex^{\text{L}}$ -- векторы-столбцы, соответствующие наблюдениям, источникам и шумам в момент времени  $t = 1, \ldots, \text{T}$, $\text{A}(\theta)$ -- матрица направляющих векторов для равномерной линейной антенной решетки:
\begin{equation*}
\text{A}(\theta) = \begin{bmatrix}
1&1&\dots&1\\
e^{-2i\pi \frac{\text{d}}{\lambda}\sin(\theta_1)}& e^{-2i\pi \frac{\text{d}}{\lambda}\sin(\theta_2)}&\dots&e^{-2i\pi \frac{\text{d}}{\lambda}\sin(\theta_\text{K})}\\
\dots&\dots&\ddots&\vdots\\
e^{-2i\pi (\text{L}-1) \frac{\text{d}}{\lambda}\sin(\theta_1)}& e^{-2i\pi (\text{L}-1) \frac{\text{d}}{\lambda}\sin(\theta_2)}&\dots&e^{-2i\pi (\text{L}-1) \frac{\text{d}}{\lambda}\sin(\theta_\text{K})}\\
\end{bmatrix},
\end{equation*}
где $d$ -- расстояние между сенсорами, $\lambda$ -- длина волны, $i$ -- мнимая единица. \\
...
Воспользуемся EM-алгоритмом (точнее, MAP-EM-алгоритмом) для того, чтобы оценить значения параметров $\Upsilon = (\theta, \mathbf{\Gamma})$,  $\Xi = (X_m, S)$ -- латентные переменные (недоступные значения и сигналы). Пусть $\Xi_t = (X_{t, m_t}, S_t)$ -- набор латентных переменных, соответствующих наблюдению $X_t$, причем для отдельных наблюдений $X_{t, m_t}=\varnothing$. Алгоритм состоит из двух шагов:
\begin{itemize}
...
\item {\bf M-шаг}
$$\Upsilon^{(\tau)}=\argmax_{\Upsilon \in \mathbb{R}^{2\text{K}}} \Expect_{q(\Xi)}[\log \Prob(X, S)] + \log \Prob(\mathbf{\Gamma}).
$$ 
\end{itemize}
