...
\end{equation*}
и $X_t \in \Complex^{\text{L}}, S_t \in \Complex^{\text{K}}, N_t \in \Complex^{\text{L}}$ -- векторы-столбцы, соответствующие наблюдениям, источникам и шумам в момент времени  $t = 1, \ldots, \text{T}$, $\text{A}$ -- матрица векторов направленности для равномерной линейной антенной решетки:
\begin{equation*}
\text{A}(\theta) = \begin{bmatrix}
1&1&\dots&1\\
e^{-2\text{i}\pi \frac{\text{d}}{\lambda}\sin(\theta_1)}& e^{-2\text{i}\pi \frac{\text{d}}{\lambda}\sin(\theta_2)}&\dots&e^{-2\text{i}\pi \frac{\text{d}}{\lambda}\sin(\theta_\text{K})}\\
\dots&\dots&\ddots&\vdots\\
e^{-2\text{i}\pi (\text{L}-1) \frac{\text{d}}{\lambda}\sin(\theta_1)}& e^{-2\text{i}\pi (\text{L}-1) \frac{\text{d}}{\lambda}\sin(\theta_2)}&\dots&e^{-2\text{i}\pi (\text{L}-1) \frac{\text{d}}{\lambda}\sin(\theta_\text{K})}\\
\end{bmatrix},
\end{equation*}
где $d$ -- расстояние между сенсорами, $\lambda$ -- длина волны, $\text{i}$ -- мнимая единица. \\

Пусть cигналы, испускаемые источниками, шумы на сенсорах и наблюдения предполагаются стохастическими: $S_t \sim \NormComplex(\mathbf{O}_{\text{K} \times 1},\mathbf{\Gamma})$, $N_t \sim \NormComplex(\mathbf{O}_{\text{L} \times 1}, \mathbf{\Lambda})$, $ X_t \sim \NormComplex(\mathbf{O}_{\text{L} \times 1}, \text{A}\mathbf{\Gamma}\text{A}^* + \mathbf{\Lambda})$, $t=1,\ldots,\text{T}$. Матрица $\mathbf{\Lambda}$ предполагается диагональной, т.е. шумы не коррелированы между собой, также в любой момент времени $S_t \perp N_t$. Рассматривается ситуация, когда известна некоторая априорная информация $\Psi$ о матрице $\mathbf{\Gamma}$. Предполагается, что $\mathbf{\Gamma} \sim \mathcal{CW}^{-1} (\Psi, \nu)$:
\begin{equation*}
\Prob(\mathbf{\Gamma} ) \propto |\mathbf{\Gamma} |^{-(\nu + \text{K})}
\exp\!\left(- \Tr(\Psi \mathbf{\Gamma} ^{-1})\right).
\end{equation*}
Составим EM-алгоритм для случая известного шума, задействуя априор на ковариацию сигналов $\mathbf{\Gamma}$.
\section{Описание алгоритма}
Воспользуемся EM-алгоритмом (точнее, MAP-EM-алгоритмом) для того, чтобы определить значения параметров $\Upsilon = (\theta, \mathbf{\Gamma})$,  $\Xi = (X_m, S)$ -- латентные переменные (недоступные значения и сигналы). Пусть $\Xi_t = (X_{t, m_t}, S_t)$ -- набор латентных переменных, соответствующих наблюдению $X_t$, причем для отдельных наблюдений $X_{t, m_t}=\varnothing$. Алгоритм состоит из двух шагов:
\begin{itemize}
\item  {\bf E-шаг}  Найти MAP-Q функцию определяемую следующим образом:
$$
Q = \Expect_{q(\Xi)}[\log \Prob(X, S)] + \log \Prob(\mathbf{\Gamma}),
$$
где 
$$
...
Требуется найти сумму математического ожидания полного правдоподобия, с учетом  текущей оценки параметров и апостериорного совместного распределения пропущенных значений в наблюдениях $X_m$ и сигналов $S$,  и штрафного слагаемого:
\begin{equation*}
 \Expect_{q(\Xi)}[\log \Prob(X, S)] + \log \Prob(\mathbf{\Gamma}) = \Expect_{(X_m,S) \mid X_o=x_o, \Upsilon^{(\tau-1)}}[\log \Prob(X, S)] + \log \Prob(\mathbf{\Gamma}).
\end{equation*}
...
\begin{equation*}
\begin{gathered}
 \Expect_{q(\Xi)}[\log \Prob(X, S)] + \log \Prob(\mathbf{\Gamma}) = \\
 \Expect_{q(\Xi)}[\log \Prob(X\mid S) + \log \Prob(S)] + \log \Prob(\mathbf{\Gamma}) = \\
\Big[\sum_{t=1}^T  \Expect_{q(\Xi_t)} \log [\Prob(X_t \mid S_t)] + \sum_{t=1}^T \Expect_{q(\Xi_t)}[\log \Prob(S_t)]\Big] + \log \Prob(\mathbf{\Gamma}) = \\
...
\begin{equation*}
\begin{gathered}
\Upsilon^{(\tau)}=\argmax_{\Upsilon} \Expect_{q(\Xi)}[\log \Prob(X, S)] + \log \Prob(\mathbf{\Gamma})= \\
...
\mathbf{\Gamma}^{(\tau)}= \argmin_{\mathbf{\Gamma}} \mathcal{Q}_2(\mathbf{\Gamma} \mid \mathbf{\Gamma}^{(\tau-1)})
 =  \argmin_{\mathbf{\Gamma}} \text{T}\bigg[ \log |\mathbf{\Gamma}| + \Tr(\mathbf{\Gamma}^{-1}\Expect_{q(\Xi)}[S S^*])\bigg] + (\nu + \text{K})\log|\mathbf{\Gamma}| + \Tr(\mathbf{\Gamma}^{-1} \Psi) \\ = \frac{\text{T} \cdot \Expect_{q(\Xi)}[S S^*] + \Psi}{\text{T} + \text{K} + \nu}.
\end{gathered}
\end{equation*}
...
\begin{enumerate}
\item
Функция $\mathcal{Q}_1(u \mid u^{(\tau-1)})$  дифференцируема, поскольку $\widetilde{\text{A}}(u)$ состоит из экспонент вида $e^{j \pi m u_k}, k = 1, \ldots, K, m \in \mathbb{R}$, т.е. гладких функций, умножение подобных матриц на фиксированные матрицы, взятие следа, взятие действительной части не нарушают гладкость.
\item
Функция $\mathcal{Q}_1(u \mid u^{(\tau-1)})$  ограничена снизу, поскольку вектор $u$ принадлежит компакту $\mathcal{U}$, матрицы, определяющие вид функции $\mathcal{Q}_1(u \mid u^{(\tau-1)})$ и отличные от $\widetilde{\text{A}}(u)$, являются фиксированными, след от произведения таких матриц, принимает лишь конечные значения.
\item
...
Оценим ковариацию сигналов $\mathbf{\Gamma}$:
\begin{equation*}
\mathbf{\Gamma}^{(\tau)}= \argmin_{\mathbf{\Gamma}} \mathcal{Q}_2(\mathbf{\Gamma} \mid \mathbf{\Gamma}^{(\tau-1)}) = \argmin_{\mathbf{\Gamma}} \text{T}\bigg[ \log |\mathbf{\Gamma}| + \Tr(\mathbf{\Gamma}^{-1}\Expect_{q(\Xi)}[S S^*])\bigg] + (\nu + \text{K})\log|\mathbf{\Gamma}| + \Tr(\mathbf{\Gamma}^{-1} \Psi).
\end{equation*}
...
\frac{\partial}{\partial \mathbf{\Gamma}}\Tr(\mathbf{\Gamma}^{-1}\Psi)= -\mathbf{\Gamma}^{-1}\Psi \mathbf{\Gamma}^{-1},  \\
\frac{\partial \mathcal{Q}_2(\mathbf{\Gamma})}{\partial \mathbf{\Gamma}} = \text{T}\mathbf{\Gamma}^{-1}- \text{T}\mathbf{\Gamma}^{-1}\Expect_{q(\Xi)}[S S^*]\mathbf{\Gamma}^{-1} + (\nu + \text{K})\mathbf{\Gamma}^{-1} -\mathbf{\Gamma}^{-1}\Psi \mathbf{\Gamma}^{-1} = \\
- \mathbf{\Gamma}^{-1}(\text{T}\Expect_{q(\Xi)}[S S^*] + \Psi) \mathbf{\Gamma}^{-1} + (\text{T} +\nu + \text{K})\mathbf{\Gamma}^{-1}.
\end{gathered}
\end{equation*}
Приравняем производную к нулю (функция по $\mathbf{\Gamma}$ выпукла):
\begin{equation*}
\mathbf{O} = (\text{T} +\nu + \text{K})\mathbf{\Gamma}^{-1} - \mathbf{\Gamma}^{-1}(\text{T}\Expect_{q(\Xi)}[S S^*] + \Psi) \mathbf{\Gamma}^{-1} \Rightarrow \mathbf{\Gamma}^{(\tau)} =  \frac{\text{T} \cdot \Expect_{q(\Xi)}[S S^*] + \Psi}{\text{T} + \text{K} + \nu}.
\end{equation*}
Ввиду того, что М-шаг не максимизирует, а лишь улучшает УМО полного правдоподобия, представленный алгоритм является не точным MAP-ЕМ, а обобщенным (Generalized) MAP-EM.
