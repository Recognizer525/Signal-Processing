...
\end{equation*}
и $X_t \in \Complex^{\text{L}}, S_t \in \Complex^{\text{K}}, N_t \in \Complex^{\text{L}}$ -- векторы-столбцы, соответствующие наблюдениям, источникам и шумам в момент времени  $t = 1, \ldots, \text{T}$, $\text{A}(\theta)$ -- матрица направляющих векторов для линейной равномерной антенной решетки размера $\text{L} \times \text{K}$:
\begin{equation*}
...
\end{equation*}
Преобразуем выражение, учитывая тот факт, что наблюдения являются независимыми, и обозначив через $\mathcal{I}$ доступную информацию на шаге $\tau$ ($X_o=x_o, \Upsilon^{(\tau-1)}$) и через $q(\Xi_t)$ распределение $\Prob(X_{t,m_t},S_t \mid X_{t,o_t}=x_{t,o_t}, \Upsilon^{(\tau-1)})$:
\begin{equation*}
\begin{gathered}
...
\end{aligned} \right.
\end{equation*}
где $\text{A}=\text{A}(\theta^{(\tau-1)})$, $\mathbf{\Gamma}=\mathbf{\Gamma}^{(\tau-1)}$, $\hat{\text{R}}=\hat{\text{R}}^{(\tau-1)}$. Для вывода этих формул используются телескопическое свойство условного математического ожидания и закон полной дисперсии (подробности приведены в приложении Б). \\
