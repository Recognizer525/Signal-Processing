...
\end{bmatrix},
\end{equation*}
где $i$ -- мнимая единица, $d$ -- расстояние между сенсорами, $\lambda$ -- длина волны, $\theta_k$ -- угловая координата $k$-го источника, $k = 1, \ldots, \mathrm{K}$. В дальнейшем $\mathbf{A}(\theta)$ будем обозначать через $\mathbf{A}$, если не оговорено иное. \\

Пусть cигналы, испускаемые источниками, шумы на сенсорах и наблюдения предполагаются стохастическими: $S_t \sim \NormComplex(\mathbf{O}_{\mathrm{K} \times 1},\mathbf{\Gamma})$, $N_t \sim \NormComplex(\mathbf{O}_{\mathrm{L} \times 1}, \mathbf{\Lambda})$, $ X_t \sim \NormComplex(\mathbf{O}_{\mathrm{L} \times 1}, \mathbf{A}\mathbf{\Gamma}\mathbf{A}^* + \mathbf{\Lambda})$, $t=1,\ldots,\mathrm{T}$. Матрица $\mathbf{\Lambda}$ предполагается диагональной, т.е. шумы не коррелированы между собой, также в любой момент времени $S_t \perp N_t$. Рассматривается ситуация, когда известна некоторая априорная информация $\Psi$ о матрице $\mathbf{\Gamma}$. Предполагается, что $\mathbf{\Gamma} \sim \mathcal{CW}^{-1} (\Psi, \nu)$:
\begin{equation*}
...
\item {\bf M-шаг}
$$\Upsilon^{(\tau)}=\argmax_{\Upsilon \in \mathbb{R}^{2\mathrm{K}}} \Expect_{q(\Xi)}[\log \Prob(X, S)] + \log \Prob(\mathbf{\Gamma}).
$$ 
\end{itemize}
Наблюдения $X_t$, $t=1,\ldots,\mathrm{T}$ предполагаются независимыми и одинаково распределенными.
...
$S_t \perp N_t$;
\item
$\mathbf{A}$ -- матрица линейного преобразования;
\item
...
\begin{equation*}
\left\{ \begin{aligned} 
\Expect[S_t \mid \mathcal{I}_t] &= \mathbf{\Gamma}^*\mathbf{A}^* \hat{\text{R}}^{-1}\Expect [X_t \mid \mathcal{I}_t], \\
\Cov(S_t \mid \mathcal{I}_t) &= \mathbf{\Gamma} - \mathbf{\Gamma}^*\mathbf{A}^*  \hat{\text{R}}^{-1}\mathbf{A}\mathbf{\Gamma} + \mathbf{\Gamma}^*\mathbf{A}^* \hat{\text{R}}^{-1}\Cov(X_t \mid \mathcal{I}_t)\hat{\text{R}}^{-1} \mathbf{A}\mathbf{\Gamma},
\end{aligned} \right.
\end{equation*}
где $\mathbf{A}=\mathbf{A}(\theta^{(\tau-1)})$, $\mathbf{\Gamma}=\mathbf{\Gamma}^{(\tau-1)}$, $\hat{\text{R}}=\hat{\text{R}}^{(\tau-1)}$. Для вывода этих формул используются телескопическое свойство условного математического ожидания и закон полной дисперсии (подробности приведены в приложении Б). \\
...
\begin{equation*}
\begin{gathered}
\Expect [X_t S_t^* \mid \mathcal{I}_t] = \Expect[X_t X_t^* \mid \mathcal{I}_t]\hat{\text{R}}^{-1}\mathbf{A}\mathbf{\Gamma},
\end{gathered}
\end{equation*}
где $\mathbf{A}=\mathbf{A}(\theta^{(\tau-1)})$, $\hat{\text{R}}=\hat{\text{R}}^{(\tau-1)}$, $\mathbf{\Gamma}=\mathbf{\Gamma}^{(\tau-1)}$. \\
...
\left\{ \begin{aligned} 
\Expect[S_t \mid \mathcal{I}_t] &= \mathbf{\Gamma}^*\mathbf{A}^* \hat{\text{R}}^{-1}\Expect [X_t \mid \mathcal{I}_t], \\
\Cov(S_t \mid \mathcal{I}_t) &= \mathbf{\Gamma} - \mathbf{\Gamma}^*\mathbf{A}^*  \hat{\text{R}}^{-1}\mathbf{A}\mathbf{\Gamma}.
\end{aligned} \right.
\end{equation*}
При этом:
\begin{equation*}
...
\end{equation*}
\begin{equation*}
\Expect [X_t S_t^* \mid \mathcal{I}_t] = \Expect[X_t \mid \mathcal{I}_t] \cdot \Expect[S_t^* \mid \mathcal{I}_t] = x_t \cdot \Expect[S_t^* \mid \mathcal{I}_t] = x_t \cdot x_t^* \cdot \hat{\text{R}}^{-1} \mathbf{A} \mathbf{\Gamma},
\end{equation*}
\begin{equation*}
\Expect [S_t S_t^* \mid \mathcal{I}_t] = \Expect[S_t \mid \mathcal{I}_t] \cdot \Expect[S_t^* \mid \mathcal{I}_t] + \Cov(S_t \mid\mathcal{I}_t). 
\end{equation*}
...
\argmin_{\Upsilon}  \mathrm{T} \Big[\log|\mathbf{\Lambda}| +\Tr \big( \mathbf{\Lambda}^{-1} \Expect_{q(\Xi)}[XX^*] \big) - 2 \Re \Tr\big(\mathbf{A}^*(\theta) \mathbf{\Lambda}^{-1} \Expect_{q(\Xi)}[X S^*]\big) \\
+ \Tr(\mathbf{\Lambda}^{-1} \mathbf{A}(\theta) \Expect_{q(\Xi)}[SS^*] \mathbf{A}^*(\theta)) + \log |\mathbf{\Gamma}| + \Tr(\mathbf{\Gamma}^{-1} \Expect_{q(\Xi)}[SS^*]) \Big] + (\nu + \mathrm{K})\log|\mathbf{\Gamma}| + \Tr(\mathbf{\Gamma}^{-1} \Psi ).
\end{gathered}
\end{equation*}
...
\begin{equation*}
\begin{gathered}
\theta^{(\tau)}= \argmin_{\theta}\mathcal{Q}_1(\theta|\theta^{(\tau-1)})  = \argmin_{\theta} \Big[- 2 \Re \Tr\big(\mathbf{A}^*(\theta) \mathbf{\Lambda}^{-1} \Expect_{q(\Xi)}[X S^*]\big) \\
...
\mathbf{\Gamma}^{(\tau)}= \argmin_{\mathbf{\Gamma}} \mathcal{Q}_2(\mathbf{\Gamma} \mid \mathbf{\Gamma}^{(\tau-1)})
 =  \argmin_{\mathbf{\Gamma}} \mathrm{T}\bigg[ \log |\mathbf{\Gamma}| + \Tr(\mathbf{\Gamma}^{-1}\Expect_{q(\Xi)}[S S^*])\bigg] + (\nu + \mathrm{K})\log|\mathbf{\Gamma}| + \Tr(\mathbf{\Gamma}^{-1} \Psi) \\ = \frac{\mathrm{T} \cdot \Expect_{q(\Xi)}[S S^*] + \Psi}{\mathrm{T} + \mathrm{K} + \nu}.
\end{gathered}
\end{equation*}
Обновляем оценку ковариации наблюдений с учетом полученных оценок параметров:
\begin{equation*}
\hat{\text{R}}^{(\tau)} = \mathbf{A}(\theta^{(\tau)})\mathbf{\Gamma}^{(\tau)}\mathbf{A}^*(\theta^{(\tau)}) + \mathbf{\Lambda}.
\end{equation*}
...
Начальную ковариацию сигналов $\mathbf{\Gamma}^{(0)}$ получаем на основе метода наименьших квадратов с помощью $\theta^{(0)}, \hat{\text{R}}^{(0)}$. Матрица $\mathbf{A}(\theta^{(0)})$ -- матрица векторов направленности и представима в следующем виде:
\begin{equation*}
...
\begin{equation*}
\check{\Gamma}^{(0)}=\mathcal{D}\big[\check{\mathbf{A}}(\theta^{(0)})^{+} (\hat{\text{R}}^{(0)}-\mathbf{\Lambda})(\check{\mathbf{A}}(\theta^{(0)})^{+})^*\big],
\end{equation*}
...
\begin{equation*}
\mathbf{\Gamma}^{(0)} - (\mathbf{\Gamma}^{(0)})^* \mathbf{A}(\theta^{(0)})^*  (\hat{\text{R}}^{(0)})^{-1} \mathbf{A}(\theta^{(0)})\mathbf{\Gamma}^{(0)} \succeq 0,
\end{equation*} 
...
= \Expect[\mathbf{\Gamma}^* \mathbf{A}^* \hat{\text{R}}^{-1} X_t \mid X_{t,o_t}=x_{t, o_t}] = \mathbf{\Gamma}^* \mathbf{A}^* \hat{\text{R}}^{-1} \Expect[X_t \mid X_{t,o_t}=x_{t, o_t}].
\end{gathered}
\end{equation*}
...
\begin{equation*}
\Expect[\Cov(S_t \mid X_t) \mid X_{t, o_t}=x_{t, o_t}] = \Cov(S_t \mid X_t) = \mathbf{\Gamma} - \mathbf{\Gamma}^* \mathbf{A}^* \hat{\text{R}}^{-1} \mathbf{A} \mathbf{\Gamma}.
\end{equation*}
Теперь преобразуем второе слагаемое:
\begin{equation*}
\begin{gathered}
\Cov(\Expect[S_t \mid X_t]\mid X_{t, o_t}=x_{t, o_t}) = \Cov(\mathbf{\Gamma}^* \mathbf{A}^* \hat{\text{R}}^{-1} X_t \mid X_{t, o_t}=x_{t, o_t})  \\ 
= \mathbf{\Gamma}^* \mathbf{A}^* \hat{\text{R}}^{-1} \Cov(X_t \mid X_{t, o_t}=x_{t, o_t}) \hat{\text{R}}^{-1} \mathbf{A} \mathbf{\Gamma}.
\end{gathered}
\end{equation*}
Таким образом:
\begin{equation*}
\Cov(S_t \mid X_{t,o_t}=x_{t, o_t}) = \mathbf{\Gamma} - \mathbf{\Gamma}^* \mathbf{A}^* \hat{\text{R}}^{-1} \mathbf{A} \mathbf{\Gamma} + \mathbf{\Gamma}^* \mathbf{A}^* \hat{\text{R}}^{-1} \Cov(X_t \mid X_{t, o_t}=x_{t, o_t}) \hat{\text{R}}^{-1} \mathbf{A} \mathbf{\Gamma}.
\end{equation*}
...
\begin{equation*}
\Expect[X_t S_t^* \mid X_t] = X_t \Expect[S_t^* \mid X_t] = X_t (\mathbf{\Gamma}^* \mathbf{A}^* \hat{\text{R}}^{-1} X_t)^* = X_t X_t^* \hat{\text{R}}^{-1} \mathbf{A} \mathbf{\Gamma},
\end{equation*}
\begin{equation*}
\begin{gathered}
\Expect[X_t S_t^* \mid X_{t, o_t}=x_{t, o_t}] = \Expect[X_t \Expect[S_t^* \mid X_t] \mid X_{t, o_t}=x_{t, o_t}] = \Expect[X_t  (\mathbf{\Gamma}^* \mathbf{A}^* \hat{\text{R}}^{-1} X_t)^*  \mid X_{t, o_t}=x_{t, o_t}] \\ 
= \Expect[X_t X_t^* \hat{\text{R}}^{-1} \mathbf{A} \mathbf{\Gamma} \mid X_{t, o_t}=x_{t, o_t}] = \Expect[X_t X_t^* \mid X_{t, o_t}=x_{t, o_t}] \hat{\text{R}}^{-1} \mathbf{A} \mathbf{\Gamma}
\end{gathered}
\end{equation*}
...
\frac{\partial \mathcal{Q}_2(\mathbf{\Gamma})}{\partial \mathbf{\Gamma}} = \mathrm{T}\mathbf{\Gamma}^{-1}- \mathrm{T}\mathbf{\Gamma}^{-1}\Expect_{q(\Xi)}[S S^*]\mathbf{\Gamma}^{-1} + (\nu + \mathrm{K})\mathbf{\Gamma}^{-1} -\mathbf{\Gamma}^{-1}\Psi \mathbf{\Gamma}^{-1} = \\
- \mathbf{\Gamma}^{-1}(\mathrm{T}\Expect_{q(\Xi)}[S S^*] + \Psi) \mathbf{\Gamma}^{-1} + (\mathrm{T} +\nu + \mathrm{K})\mathbf{\Gamma}^{-1}.
\end{gathered}
\end{equation*}
Приравняем производную к нулю (функция по $\mathbf{\Gamma}$ выпукла):
\begin{equation*}
\mathbf{O} = (\mathrm{T} +\nu + \mathrm{K})\mathbf{\Gamma}^{-1} - \mathbf{\Gamma}^{-1}(\mathrm{T}\Expect_{q(\Xi)}[S S^*] + \Psi) \mathbf{\Gamma}^{-1} \Rightarrow \mathbf{\Gamma}^{(\tau)} =  \frac{\mathrm{T} \cdot \Expect_{q(\Xi)}[S S^*] + \Psi}{\mathrm{T} + \mathrm{K} + \nu}.
\end{equation*}
