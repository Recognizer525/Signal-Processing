...
Воспользуемся EM-алгоритмом для того, чтобы определить значения параметров $\Upsilon = (\theta, \mathbf{\Gamma})$,  $\Xi = (X_m, S)$ -- латентные переменные (недоступные значения и сигналы). Пусть $\Xi_t = (X_{t, m_t}, S_t)$ -- набор латентных переменных, соответствующих наблюдению $X_t$, причем для отдельных наблюдений $X_{t, m_t}=\varnothing$.
\begin{center}
\bf EM-алгоритм
\end{center}
\begin{itemize}
\item  {\bf E-шаг}  Найти математическое ожидание полоного правдоподобия с учетом апосториорного распределения латентных переменных:
$$
...
\begin{equation}
 \Expect_{(X_m,S)|X_o=x_o, \Upsilon^{(\tau-1)}}[\log \Prob(X, S)].
\end{equation}
...
\begin{gathered}
 \Expect_{q(\Xi)}[\log \Prob(X, S)] = \\
 \Expect_{q(\Xi)}[\log \Prob(X|S) + \log \Prob(S)] = \\
\Big[\sum_{t=1}^T  \Expect_{(X_{t,m_t},S_t)|\mathcal{I}_t} \log [\Prob(X_t|S_t)] + \sum_{t=1}^T \Expect_{(X_{t,m_t},S_t)|\mathcal{I}_t}[\log \Prob(S_t)]\Big] = \\
...
\end{gathered}
\end{equation*}
Выражение $\Expect_{(X_{t,m_t},S_t)|\mathcal{I}_t}[\cdot]$  соответствует случаю максимально полного набора латентных переменных: некоторые наблюдения в наборе могут не содержать пропуски ($X_{t,m_t}=\varnothing$), и тогда это выражение упрощается до $\Expect_{S_t|\mathcal{I}_t}[\cdot]$.
...
Распределение $\Prob(X_{t,m_t},S_t|X_{t,o_t})$ задается однозначно ввиду того, что:
\begin{enumerate}
\item
$S_t \perp N_t$;
\item
...
\end{equation*}
Для каждого наблюдения, содержащего пропуски, требуется найти апостериорное распределение пропущенных значений, $\Prob(X_{t, m_t}|X_{t, o_t}=x_{t, o_t},\Upsilon^{(\tau-1)}), t=1,\ldots,\text{T}, m_t \ne \varnothing$ . Обозначим через $\mathcal{I}_t$ условную информацию $X_{t, o_t}=x_{t, o_t},\Upsilon^{(\tau-1)}$.
Параметры апостериорного распределения $\Prob(X_{t, m_t}|\mathcal{I}_t), t=1,\ldots,\text{T}$ на итерации $\tau$ можно найти следующим образом:
\begin{equation}
\left\{ \begin{aligned} 
\Expect[X_{t, m_t}|\mathcal{I}_t] &= \hat{\text{R}}_{m_t, o_t}\left(\hat{\text{R}}_{o_t, o_t}\right)^{-1}\cdot x_{t, o_t}, \\
\Cov(X_{t, m_t}|\mathcal{I}_t) &= \hat{\text{R}}_{m_t, m_t}-\hat{\text{R}}_{m_t, o_t}\left(\hat{\text{R}}_{o_t, o_t}\right)^{-1}\hat{\text{R}}_{o_t, m_t},
\end{aligned} \right.
\end{equation}
...
\begin{equation*}
\begin{gathered}
\Expect[X_t X_t^*|\mathcal{I}_t] = \Expect [X_t | \mathcal{I}_t] \cdot \Expect [X_t^* | \mathcal{I}_t] + \Cov (X_t | \mathcal{I}_t) \\
= \Expect [X_t | \mathcal{I}_t] \cdot \Expect [X_t^* | \mathcal{I}_t] +
\begin{pmatrix}
\mathbf{O}_{o_t, o_t} & \mathbf{O}_{o_t, m_t} \\
\mathbf{O}_{m_t, o_t} & \Cov(X_{t, m_t}|\mathcal{I}_t) \\
\end{pmatrix},
\end{gathered}
\end{equation*}
...
Параметры апостериорного распределения $P(S_t|\mathcal{I}_t), t = 1,\ldots, \text{T}$ можно найти следующим образом:
\begin{equation}
\left\{ \begin{aligned} 
\Expect[S_t|\mathcal{I}_t] &= \mathbf{\Gamma}^*\text{A}^* \hat{\text{R}}^{-1}\Expect [X_t | \mathcal{I}_t], \\
\Cov(S_t|\mathcal{I}_t) &= \mathbf{\Gamma} - \mathbf{\Gamma}^*\text{A}^*  \hat{\text{R}}^{-1}\text{A}\mathbf{\Gamma} + \mathbf{\Gamma}^*\text{A}^* \hat{\text{R}}^{-1}\Cov(X_t|\mathcal{I}_t)\hat{\text{R}}^{-1} \text{A}\mathbf{\Gamma},
\end{aligned} \right.
\end{equation}
...
\begin{equation}
\Expect_{(X_{t,m_t},S_t)|\mathcal{I}_t}[S_t S_t^*] = \Expect[S_t|\mathcal{I}_t] \cdot \Expect[S_t^*|\mathcal{I}_t] + \Cov(S_t|\mathcal{I}_t),
\end{equation}
...
Если $m_t = \varnothing$, то апостериорное распределение упрощается следующим образом: $\Prob(X_{t,m_t},S_t|\mathcal{I}_t)=\Prob(S_t|\mathcal{I}_t)$.
Параметры апостериорного распределения $\Prob(S_t|\mathcal{I}_t), t = 1,\ldots, \text{T}$:
\begin{equation}
\left\{ \begin{aligned} 
\Expect[S_t|\mathcal{I}_t] &= \mathbf{\Gamma}^*\text{A}^* \hat{\text{R}}^{-1}\Expect [X_t | \mathcal{I}_t], \\
\Cov(S_t|\mathcal{I}_t) &= \mathbf{\Gamma} - \mathbf{\Gamma}^*\text{A}^*  \hat{\text{R}}^{-1}\text{A}\mathbf{\Gamma}.
\end{aligned} \right.
\end{equation}
При этом:
\begin{equation}
...
\end{equation}
\begin{equation}
\Expect_{S_t|\mathcal{I}_t}[S_t S_t^*] = \Expect[S_t|\mathcal{I}_t] \cdot \Expect[S_t^*|\mathcal{I}_t] + \Cov(S_t|\mathcal{I}_t). 
\end{equation}
\clearpage
\subsection{E-шаг, Агрегация}
Теперь оценим вторые начальные моменты  $\Expect_{q(\Xi)}[X X^*]$,  $\Expect_{q(\Xi)}[S S^*]$, $\Expect_{q(\Xi)}[X S^*]$:
\begin{equation}
\Expect_{q(\Xi)}[XX^*] = \frac{1}{\text{T}}\sum_{t=1}^T \Expect[X_t X_t^*|\mathcal{I}_t],
\end{equation}
\begin{equation}
...
\begin{figure}[h]
    ...
    \caption{График роста $\log \Prob(X_o|\Upsilon)$ для первого набора начальных условий}
\end{figure}
\clearpage
...
\begin{figure}[h]
    ...
    \caption{График роста $\log \Prob(X_o|\Upsilon)$ для второго набора начальных условий}
\end{figure}
\clearpage
...
В рамках Е-шага необходимо вычислить $\Expect[S_t|X_{t,o_t}=x_{t, o_t}]$,  $\Cov(S_t|X_{t,o_t}=x_{t, o_t})$.
...
\end{gathered}
\end{equation*}
Для нахождения $\Cov(S_t|X_{t,o_t}=x_{t, o_t})$ может быть использовано равенство, называемое законом полной дисперсии (Law of total variance, может быть найдено в Sheldon Russ, A first course in probability, 8th ed., 2010):
\begin{equation}
\Cov(W) = \Expect[\Cov(W|Y)] + \Cov(\Expect[W|Y]).
\end{equation}
...
\begin{equation}
\Cov(W|\mathcal{F}_Z) = \Expect[\Cov(W|Y,Z)|\mathcal{F}_Z] + \Cov(\Expect[W|Y,Z]|\mathcal{F}_Z).
\end{equation}
...
Получаем:
\begin{equation}
\Cov(W|\mathcal{F}_Z) = \Expect[\Cov(W|Y)|\mathcal{F}_Z] + \Cov(\Expect[W|Y]|\mathcal{F}_Z).
\end{equation}
Если $Z=z$, имеем:
\begin{equation}
\Cov(W|Z=z) = \Expect[\Cov(W|Y)|Z=z] + \Cov(\Expect[W|Y]|Z=z).
\end{equation}
...
Получаем:
\begin{equation}
\Cov(S_t|X_{t,o_t}=x_{t, o_t}) = \Expect[\Cov(S_t|X_t)|X_{t,o_t}=x_{t, o_t}] + \Cov(\Expect[S_t|X_t]|X_{t,o_t}=x_{t, o_t}).
\end{equation}
Преобразуем первое слагаемое, учитывая тот факт, что ковариация не зависит от реализации $X_{t, o_t}$.
\begin{equation}
\Expect[\Cov(S_t|X_t)|X_{t, o_t}=x_{t, o_t}] = \Cov(S_t|X_t) = \mathbf{\Gamma} - \mathbf{\Gamma}^* \text{A}^* \hat{\text{R}}^{-1} \text{A} \mathbf{\Gamma}.
\end{equation}
Теперь преобразуем второе слагаемое:
\begin{equation*}
\begin{gathered}
\Cov(\Expect[S_t|X_t]|X_{t, o_t}=x_{t, o_t}) = \Cov(\mathbf{\Gamma}^* \text{A}^* \hat{\text{R}}^{-1} X_t |X_{t, o_t}=x_{t, o_t})  \\ 
= \mathbf{\Gamma}^* \text{A}^* \hat{\text{R}}^{-1} \Cov(X_t | X_{t, o_t}=x_{t, o_t}) \hat{\text{R}}^{-1} \text{A} \mathbf{\Gamma}.
\end{gathered}
\end{equation*}
Таким образом:
\begin{equation}
\Cov(S_t|X_{t,o_t}=x_{t, o_t}) = \mathbf{\Gamma} - \mathbf{\Gamma}^* \text{A}^* \hat{\text{R}}^{-1} \text{A} \mathbf{\Gamma} + \mathbf{\Gamma}^* \text{A}^* \hat{\text{R}}^{-1} \Cov(X_t | X_{t, o_t}=x_{t, o_t}) \hat{\text{R}}^{-1} \text{A} \mathbf{\Gamma}.
\end{equation}
Для полных наблюдений $X_{t, o_t} = X_t$, и, соответственно:
\begin{equation}
\Cov(\Expect[S_t|X_t]|X_{t, o_t}=x_{t, o_t})  = \Cov(\Expect[S_t|X_t]|X_t=x_t) = 0. 
\end{equation}
...
\begin{equation}
\mathcal{Q}_1(u|u^{\tau-1)}) = ||\mathbf{\Lambda}^{-1/2} (\Expect_{q(\Xi)}[X S^*]-\widetilde{\text{A}}(u) \Expect_{q(\Xi)}[S S^*]) ||_F^2,
\end{equation}
где
\begin{equation}
{\small
...
\begin{equation}
\mathbf{\Gamma}^{(\tau)}= \argmin_{\mathbf{\Gamma}} \mathcal{Q}_2(\mathbf{\Gamma} | \mathbf{\Gamma}^{(\tau-1)}) = \text{T}\bigg[ \log |\mathbf{\Gamma}| + \Tr(\mathbf{\Gamma}^{-1}\Expect_{q(\Xi)}[S S^*])\bigg].
\end{equation}
