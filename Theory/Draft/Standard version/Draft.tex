...
\item {\bf M-шаг}
$$\Upsilon^{(\tau)}=\argmax_{\Upsilon \in \mathbb{R}^{2\mathrm{K}}} \Expect_{q(\Xi)}[\log \Prob(X, S)].
$$ 
\end{itemize}
...
\mathbf{\Gamma}^{(\tau)}= \argmin_{\mathbf{\Gamma}} \mathcal{Q}_2(\mathbf{\Gamma} \mid \mathbf{\Gamma}^{(\tau-1)})
 =  \argmin_{\mathbf{\Gamma}} \mathrm{T}\bigg[ \log |\mathbf{\Gamma}| + \Tr(\mathbf{\Gamma}^{-1}\Expect_{q(\Xi)}[S S^*])\bigg] = \Expect_{q(\Xi)}[S S^*] .
\end{gathered}
\end{equation*}
