...
\end{center}
Вектор направлений прибытия сигналов $\theta$ инициализируем реализацией случайной величины из многомерного равномерного распределения. Каждая компонента $\theta_m$ вектора $\theta, m = \overline{1,M}$ принимает значения на отрезке $[-\pi; \pi]$. Начальные оценки сигнала получаем следующим образом: найдем подмножество снимков $ X_{full}$ полученного сигнала, состоящих только из наблюдаемых данных, а также оценим матрицу $A$ на основании инициализованного вектора $\theta$, а затем найдем псевдообратную $A^+ $ к этой матрице, после чего находим наилучшие оценки в смысле МНК:
\begin{equation}
\hat{S} = A^+ X_{full}.
\end{equation}
\begin{center}
