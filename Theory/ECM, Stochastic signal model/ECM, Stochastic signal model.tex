...
\nonumber
\end{gather}
Сигналы, испускаемые источниками, также как и шумы на сенсорах, предполагаются стохастическими: $S_t \sim CN(\mathbf{O}_{K \times 1},\mathbf{P}),t=\overline{1,G}$, $N_t \sim CN(\mathbf{O}_{L \times 1}, \mathbf{\Lambda})$. Матрицы $\mathbf{P}$ и $\mathbf{\Lambda}$ предполагаются диагональными, т.е. и сигналы, и шумы, являются некоррелированными. Для простоты дальнейших рассуждений введем также следующие величины:
\begin{itemize}
\item
...
$\widetilde{x}_{m_t} = \mu_{X_{m_t}|X_{o,t}=x_{o_t}}$. $\widetilde{x}_t^{(\tau)}$ --- оценка $x_t$ с учетом оценки пропусков.
...
\begin{equation*}
\begin{gathered}
\log P(X,S|\theta, \mathbf{P}) = \log P(X|S=s, \theta) + \log P(S|\mathbf{P}) = \\
 = -G \log (\Det(\pi \mathbf{\Lambda})) - \sum_{t=1}^G (X_t-A(\theta)S_t)^H \mathbf{\Lambda}^{-1}(X_t-A(\theta)S_t) 
...
\begin{equation}
\theta^{(\tau)}=\argmin_{\theta} ||\mathbf{\Lambda}^{-1/2}(\widetilde{x}^{(\tau)}-AS)||_F^2
\end{equation}
\begin{center}
