...
\begin{equation}
\left\{ \begin{aligned} 
\mu_{S_t|\mathcal{I}_t, X_{t, m_t}} &= \mathbf{\Gamma}\text{A}^* \hat{\text{R}}^{-1}\Expect[X_t|\mathcal{I}_t], \\
...
= \widetilde{\Sigma}_{X_t} (\hat{\text{R}}^{(\tau-1)})^{-1}\text{A}(\theta^{(\tau-1)})\mathbf{\Gamma}^{(\tau-1)} + \Expect[X_t|\mathcal{I}_t] \cdot \Expect[S_t|\mathcal{I}_t]^*.
\end{gathered}
\end{equation}
\begin{equation}
\begin{gathered}
