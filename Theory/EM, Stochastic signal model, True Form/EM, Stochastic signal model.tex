...
\end{equation}
Преобразуем выражение, учитывая тот факт, что наблюдения являются независимыми, и обозначив через $\mathcal{I}$ условную информацию $X_o=x_o, \Psi^{(\tau-1)}$ и через $\mathcal{I}_t$ условную информацию $X_{t,o_t}=x_{t,o_t}, \Psi^{(\tau-1)}$:
\begin{equation*}
\begin{gathered}
...
\end{gather}
Для наблюдений, не содержащих пропуски, апостериорное распределение упрощается следующим образом: $P(X_{t,m_t},S_t|\mathcal{I}_t)=P(S_t|\mathcal{I}_t)$.
...
Для каждого наблюдения $X_t$, содержащего пропуски, оценим условный второй начальный момент $\widetilde{\Sigma}_{X_t} =\Expect_{X_{m_t}|\mathcal{I}_t}[X_t X_t^*]$:
\begin{equation*}
\begin{gathered}
...
Оценим условный второй начальный момент для набора наблюдений $\widetilde{\Sigma}_X = \Expect_{(X_m,S)|\mathcal{I}}[XX^*]$:
\begin{equation}
...
\end{aligned} \right.
\end{equation}
Оценим условный второй начальный момент для  набора сигналов с учетом текущей оценки параметров и доступных наблюдений $\widetilde{\Sigma}_S = \Expect_{(X_m,S)|\mathcal{I}}[SS^*]$. \\
\begin{equation}
...
\end{equation}
Остается величину $\widetilde{\Sigma}_{XS}=\Expect_{(X_m,S)|\mathcal{I}}[X S^*]$:
\begin{equation}
\begin{gathered}
\Expect_{(X_{t,m_t},S_t)|\mathcal{I}_t}[X_t S_t^*] = \Expect_{(X_{t,m_t},S_t)|\mathcal{I}_t}[X_tX_t^*] \hat{\text{R}}^{-1}\text{A}\mathbf{\Gamma} + \Expect[X_t|\mathcal{I}_t] \cdot \Expect[S_t|\mathcal{I}_t]^*\\
= \widetilde{\Sigma}_{X_t} \hat{\text{R}}^{-1}\text{A}\mathbf{\Gamma} +  \mu_{X_t|\mathcal{I}_t}\cdot \mu_{S_t|\mathcal{I}_t, X_{t, m_t}}^*,
\end{gathered}
\end{equation}
\begin{equation}
\begin{gathered}
\widetilde{\Sigma}_{XS}= \Expect_{(X_m,S)|\mathcal{I}}[X S^*] = \frac{1}{\text{T}} \sum_{t=1}^T \Big( \widetilde{\Sigma}_{X_t} \hat{\text{R}}^{-1}\text{A}\mathbf{\Gamma} +  \mu_{X_t|\mathcal{I}_t}\cdot \mu_{S_t|\mathcal{I}_t, X_{t, m_t}}^* \Big),
\end{gathered}
\end{equation}
...
\mathbf{\Gamma}^{(\tau)}= \argmin_{\mathbf{\Gamma}} \mathcal{K}(\mathbf{\Gamma} | \mathbf{\Gamma}^{(\tau-1)})
 = \text{T}\bigg[ \log |\mathbf{\Gamma}| + \Tr(\mathbf{\Gamma}^{-1}\widetilde{\Sigma}_S)\bigg].
\end{gathered}
\end{equation*}
...
\frac{\partial}{\partial \mathbf{\Gamma}}\Tr(\mathbf{\Gamma}^{-1}\widetilde{\Sigma}_S)= -\mathbf{\Gamma}^{-1}\widetilde{\Sigma}_S\mathbf{\Gamma}^{-1}, \\
\frac{\partial \mathcal{K}(\mathbf{\Gamma})}{\partial \mathbf{\Gamma}} = \mathbf{\Gamma}^{-1}-\mathbf{\Gamma}^{-1}\widetilde{\Sigma}_S\mathbf{\Gamma}^{-1}.
\end{gathered}
\end{equation*}
Приравняем производную к нулю (функция по $\mathbf{\Gamma}$ выпукла):
\begin{equation*}
O = \mathbf{\Gamma}^{-1}-\mathbf{\Gamma}^{-1}\widetilde{\Sigma}_S\mathbf{\Gamma}^{-1} \Rightarrow \widetilde{\Sigma}_S = \mathbf{\Gamma} \Rightarrow \mathbf{\Gamma}^{(\tau)} = \widetilde{\Sigma}_S.
\end{equation*}
