...
\begin{equation}
\begin{gathered}
\theta^{(\tau+1)}=\argmax_{\theta} E[\log P(X_o, X_m, \, S|\theta^{(\tau)})|X_o=x_o, \theta^{(\tau)}] =\\
\argmax_{\theta} E\left[\sum_{t=1}^G\log P(X_{o,t}, X_{m,t}, \, S_t|\theta^{(\tau)})|X_{o,t}=x_{o,t}, \theta^{(\tau)}\right]
\end{gathered}
\end{equation}
...
\end{gather}
\begin{gather*}
 E\left[(Y_t-\mu_{Y_t})^H\Sigma_{Y_t}^{-1}(Y_t-\mu_{Y_t})|X_{o,t}=x_{o,t},\theta^{(\tau)}\right]  =  E\left[[[\Sigma_{Y_t}^{-1}]^{\frac{1}{2}}(Y_t-\mu_{Y_t})]^H[\Sigma_{Y_t}^{-1}]^{\frac{1}{2}}(Y_t-\mu_{Y_t})|X_{o,t}=x_{o,t},\theta^{(\tau)}\right] =\\
= \Tr(E\left[[\Sigma_{Y_t}^{-1}]^{\frac{1}{2}}(Y_t-\mu_{Y_t})[[\Sigma_{Y_t}^{-1}]^{\frac{1}{2}}(Y_t-\mu_{Y_t})]^H|X_{o,t}=x_{o,t},\theta^{(\tau)}\right])=\\
