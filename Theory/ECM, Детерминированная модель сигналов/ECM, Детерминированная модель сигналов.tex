...
\begin{equation}
\left\{ \begin{aligned} 
K_{x_{o_t},x_{o_t}} &= \mathbf{\Lambda}_{o_t} \\
K_{x_{o_t},x_{m_t}} &= \hat{K}_{x_{o_t},x_{m_t}} \\
K_{x_{m_t},x_{o_t}} &= \hat{K}^H_{x_{o_t},x_{m_t}} \\
K_{x_{m_t},x_{m_t}} &= \mathbf{\Lambda}_{m_t} \\
m_{x_{o_t}} &= A_{o_t}S_t \\
m_{x_{m_t}} &= A_{m_t}S_t
\end{aligned} \right.
\end{equation}
где $\hat{K}_{x_{o_t},x_{m_t}}$ -- выборочная оценка ковариации на основе полных наблюдений, т.е. таких, что $x_t = x_{o_t}$
\begin{equation}
\left\{ \begin{aligned} 
m_{x_{m_t}|x_{o_t}} &= A_{m_t}S_t + \hat{K}^H_{x_{o_t},x_{m_t}}(\mathbf{\Lambda}_{o_t})^{-1}\cdot(x_o-A_{o_t}S_t) \\
K_{x_{m_t}|x_{o_t}} &= \mathbf{\Lambda}_{m_t}-\hat{K}^H_{x_{o_t},x_{m_t}}(\mathbf{\Lambda}_{o_t})^{-1}\hat{K}_{x_{o_t},x_{m_t}},
\end{aligned} \right.
\end{equation}
...
\argmax_{\Psi}  \Expect_{X_m|X_o=x_o, \Psi^{(\tau-1)}}\left[\sum_{t=1}^G\log P(X_{o_t}, X_{m_t})\right] = \\
