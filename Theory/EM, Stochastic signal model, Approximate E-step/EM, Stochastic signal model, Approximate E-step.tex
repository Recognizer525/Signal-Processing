...
Оценим ковариационную матрицу наблюдений с учетом текущей оценки параметров и доступных наблюдений $\widetilde{\Sigma}_X = \Expect_{\mathcal{I}}[XX^*]$ (действуем по аналогии с М-шагом для стандартного ЕМ-алгоритма оценки параметров гауссовского распределения по выборке из одинаково распределенных наблюдений, содержащих пропуски). \\
\begin{equation}
\widetilde{\Sigma}_X = \frac{1}{\text{T}} \hat{x} \hat{x}^*  + \frac{1}{\text{T}} \Big( \sum_{t=1}^\text{T} \widetilde{R}_t \Big),
\end{equation}
где $\widetilde{R}_t$ -- матрица, в которой элементы $(\widetilde{R}_t)_{ij}$, такие что $i,j \in m_t$ образуют $\Sigma_{X_{t, m_t}|\mathcal{I}_t}^{(\tau)}$, а остальные элементы равны нулю, $\hat{x}$ -- реализация $x$ матрицы наблюдений $X$, в которой значения $x_{t, m_t}$ оценены как $\hat{x}_{t, m_t}=\mu_{X_{t, m_t}|\mathcal{I}_t}^{(\tau)}$ для всех $t \in \{1,\ldots,\text{T}\}$.
Параметры апостериорного распределения $P(S_t|\mathcal{I}_t), t = 1,\ldots, \text{L}$ можно найти исходя из следующих формул:
\begin{equation}
\left\{ \begin{aligned} 
\mu_{S_t|\mathcal{I}_t, X_{t, m_t}}^{(\tau)} &= \mathbf{\Gamma}\text{A}^*\Big(\widetilde{\Sigma}_X\Big)^{-1}\hat{x}_t, \\
\Sigma_{S|\mathcal{I}_t, X_{t, m_t}}^{(\tau)} &= \mathbf{\Gamma} - \mathbf{\Gamma}\text{A}^*\Big(\widetilde{\Sigma}_X\Big)^{-1}\text{A}\mathbf{\Gamma},
\end{aligned} \right.
\end{equation}
...
\begin{equation}
\widetilde{\Sigma}_S = \frac{1}{\text{\text{T}}} \hat{s} \hat{s}^*  + \frac{1}{\text{T}} \Big( \sum_{t=1}^T \Sigma_{S_t|\mathcal{I}_t, X_{t, m_t}}^{(\tau)} \Big),
\end{equation}
где $\hat{s} = [ \mu_{S_1|\mathcal{I}_1, X_{1, m_1}},\ldots, \mu_{S_\text{L}|\mathcal{I}_\text{L}, X_{\text{L}, m_\text{L}}}]$. Заметим, что $\Sigma_{S_t|\mathcal{I}_t, X_{t, m_t}}$ не зависит от $t$.
Остается оценить кросс-ковариацию $\widetilde{\Sigma}_{XS}=\Expect_{(X_m,S)|\mathcal{I}_t}[X S^*]$:
\begin{equation}
\begin{gathered}
