...
def incomplete_lkhd(X, theta, P, Q):
    ...
    R = A @ P @ A.conj().T + Q
    R = 0.5 * (R + R.conj().T) + 1e-6 * np.eye(Q.shape[0])
...
def ECM(theta: np.ndarray, P: np.ndarray, X: np.ndarray, Q: np.ndarray, max_iter: int=50, eps: float=1e-6):
    """
    ...
    while ECM_Iteration < max_iter:
        ...
        #print(f'diff of P is {np.sum((new_P-P)**2)} on iteration {ECM_Iteration}')
        theta, P = new_theta, new_P
        lkhd = incomplete_lkhd(X_modified, theta, P, Q)
        print(f'likelihood is {lkhd.real} on iteration {ECM_Iteration}')

        ECM_Iteration += 1
    return theta, lkhd


def multi_start_ECM(X: np.ndarray, M: int, Q: np.ndarray, num_of_starts: int = 30, max_iter: int = 20, eps: float = 1e-6):
    """
    ...
    for i in range(num_of_starts):
        ...
        est_theta, est_lhd = ECM(theta, P, X, Q, max_iter, eps)
        if est_lhd > best_lhd:
