...
def ECM(theta: np.ndarray, S: np.ndarray, X: np.ndarray, Q: np.ndarray, max_iter: int=50, eps: float=1e-6):
    """
...
def multi_start_ECM(X: np.ndarray, M: int, Q: np.ndarray, num_of_starts: int = 30, max_iter: int = 20, eps: float = 1e-6):
    ...
    for i in range(num_of_starts):
        ...
        est_theta, est_lhd = ECM(theta, S, X, Q, max_iter, eps)
        if est_lhd > best_lhd:
