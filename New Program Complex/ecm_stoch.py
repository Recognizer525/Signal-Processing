...
def ECM(theta: np.ndarray, P: np.ndarray, X: np.ndarray, Q: np.ndarray, max_iter: int=50, eps: float=1e-6):
    ...
    Параметры:
    theta: np.ndarray - вектор углов, которые соответствуют DOA;
    P: np.ndarray - ковариация исходных сигналов;
    X: np.ndarray - коллекция полученных сигналов;
    Q: np.ndarray - ковариация шума;
    max_iter: int - предельное число итерация;
    eps: float - величина, используемая для проверки сходимости последних итераций.
    ...
    while EM_Iteration < max_iter:
        ...
        K = sensors.robust_complex_cov(X_modified) + K_Xm_cond_accum / G
