...
def init_est(X: np.ndarray, 
    ...
    S, _ = sensors.gds(K, len(X), seed=seed+20)
    return theta, S
...
def multi_start_ECM_kn(X: np.ndarray, 
    ...
    """
    all_thetas = {}
    best_lhd, best_theta, best_S, best_start = -np.inf, None, None, None
    for i in range(num_of_starts):
        print(f'{i}-th start')
        theta, S = init_est(X, K, seed=i * 100)
        est_theta, est_S, est_lhd, thetas = ECM_kn(theta=theta, S=S, X=X, Q=Q, max_iter=max_iter, rtol=rtol)
        all_thetas[i] = thetas
        if est_lhd > best_lhd:
            best_lhd, best_theta, best_S, best_start = est_lhd, est_theta, est_S, i
    best_theta = sensors.angle_correcter(best_theta)
    print(f'best_start={best_start}')
    return best_theta, best_S, best_lhd, all_thetas
